#!/usr/bin/env python3
"""
LoreOut Completionist Map Generator v4 -- Clean Custom Icons
==============================================================
Generates a professional completionist world map overlay for Fallout 4
matching the Aeon Commonwealth map aesthetic.

All icons are drawn programmatically -- no external assets needed.
Clean, small, white icons with tiny category-colored text labels.
Collectibles shown as small grouped symbols near their locations.

Features:
  - Custom-drawn icon set: house, vault gear, star, ?, M, buildings, etc.
  - All icons rendered via 4x supersample for crisp anti-aliasing
  - Small fonts for clean, professional appearance
  - Gold dashed grid with alphanumeric labels
  - Dashed playable area border
  - Legend in top-left corner
  - Collectible indicators grouped near parent locations
  - Bahnschrift font throughout
...
# Amber/olive dashed grid -- military map style
GRID_COLOR = (180, 165, 70, 45)
GRID_LABEL_COLOR = (190, 175, 80, 150)
...
# Location icon sizes (on 8K map) -- kept small and tactical
ICON_SIZE = 20       # standard location icon
ICON_SIZE_MAJOR = 24 # cities, vaults -- high priority targets
ICON_SIZE_SMALL = 14 # POI / other -- recon markers

# Collectible indicator size
COLLECT_ICON_SIZE = 8
...
# =============================================================================
# CUSTOM ICON DRAWING -- All icons drawn programmatically at 4x then downscaled
# =============================================================================
_icon_cache = {}
...
def _draw_poi(d, cx, cy, s, color, offset=0):
    ...
    # Short crosshair ticks (not full lines -- more recon-style)
    tick = int(r * 0.4)
...
def _draw_metro(d, cx, cy, s, color, offset=0):
    ...
    # Circle
    d.ellipse([cx-r, cy-r, cx+r, cy+r], outline=color, width=lw)
    # Horizontal bar through middle (underground passage)
    bar_h = max(2, lw)
    d.rectangle([cx - r + lw, cy - bar_h, cx + r - lw, cy + bar_h], fill=color)

def _draw_city(d, cx, cy, s, color, offset=0):
    """City -- Urban area: angular skyline silhouette (tactical style)."""
    o = offset
    hs = s // 2 + o
    # Angular skyline -- three blocks of different heights
    bw = max(2, s // 5)
    gap = max(1, s // 16)
    # Tall center tower
...
def _draw_police(d, cx, cy, s, color, offset=0):
    """Police -- 6-pointed star badge (law enforcement)."""
    o = offset
    r = s // 2 + o
    ri = int(r * 0.5)
    pts = []
...
# --- Collectible mini-icons (military tactical style) ---
...
# Build icon generators lookup -- Military / STALKER tactical symbols
ICON_GENERATORS = {
    'settlement': _draw_settlement,    # Flag/pennant (friendly base)
    'vault':      _draw_vault,         # Crosshair circle (hardened bunker)
    'military':   _draw_military,      # NATO unit box with X (combat)
    'metro':      _draw_metro,         # Circle with bar (underground)
    'city':       _draw_city,          # Angular skyline (urban area)
    'police':     _draw_police,        # 6-point star (law enforcement)
    'other':      _draw_waypoint,      # Chevron (generic waypoint)
    'poi':        _draw_poi,           # Crosshair target (recon marker)
}

COLLECT_GENERATORS = {
    'bobblehead':   _draw_mini_star,     # High-value target
    'magazine':     _draw_mini_intel,    # Intel document
    'power_armor':  _draw_mini_shield,   # Equipment cache
    'fusion_core':  _draw_mini_dot,      # Energy resource
    'trader':       _draw_mini_diamond,  # Commerce point
}

def get_icon(category, size):
...
def get_collect_icon(category, size):
...
def draw_dashed_line(draw, start, end, color, width=1, dash_len=20, gap_len=15):
...
def draw_grid(draw, font_grid):
    ...
    while wx <= end_x:
        ...
        if 60 < px < IMG_SIZE - 60:
            draw_dashed_line(draw, (px, 0), (px, IMG_SIZE), GRID_COLOR, width=1)
            if col_idx < len(col_labels):
                draw.text((px - 10, 12), col_labels[col_idx],
    ...
    while wy >= start_y:
        ...
        if 60 < py < IMG_SIZE - 60:
            draw_dashed_line(draw, (0, py), (IMG_SIZE, py), GRID_COLOR, width=1)
            draw.text((12, py - 8), f"{row_idx:02d}",
                      font=font_grid, fill=GRID_LABEL_COLOR)
        row_idx += 1
        wy -= GRID_SPACING
...
class LabelPlacer:
    ...
    def try_place(self, x, y, w, h, max_attempts=6):
        offsets = [(0, 0), (0, h+2), (0, -(h+2)), (w+4, 0), (-(w+4), 0), (0, h*2+4)]
...
# TEXT HELPERS
# =============================================================================
def draw_text_shadow(draw, pos, text, font, fill=WHITE, shadow=(0, 0, 0, 140)):
    x, y = pos
    draw.text((x + 1, y + 1), text, font=font, fill=shadow)
    draw.text((x, y), text, font=font, fill=fill)

# =============================================================================
# COLLECTIBLE GROUPING -- find nearest location for each collectible
# =============================================================================
def group_collectibles_to_locations(collectibles, locations, max_dist=300):
    ...
    Returns dict: location_index -> list of collectible categories nearby.
    """
    groups = {}
...
# LEGEND
# =============================================================================
def draw_legend(overlay, font_legend, font_legend_title):
    """Draw a semi-transparent legend box in the top-left corner."""
    legend_items = [
        ('Settlement', 'settlement', ICON_GENERATORS['settlement']),
        ('Bunker / Vault', 'vault', ICON_GENERATORS['vault']),
        ('Urban Zone', 'city', ICON_GENERATORS['city']),
        ('Military Site', 'military', ICON_GENERATORS['military']),
        ('Underground', 'metro', ICON_GENERATORS['metro']),
        ('Authority Post', 'police', ICON_GENERATORS['police']),
        ('Recon Target', 'poi', ICON_GENERATORS['poi']),
    ]
    collect_items = [
        ('Bobblehead', 'bobblehead'),
        ('Intel (Magazine)', 'magazine'),
        ('Equipment Cache', 'power_armor'),
        ('Energy Cell', 'fusion_core'),
        ('Trader', 'trader'),
    ]
    ...
    row_h = 30
    box_w = 300
    box_h = 50 + len(legend_items) * row_h + 30 + len(collect_items) * row_h + 20

    # Semi-transparent background
    legend_bg = Image.new('RGBA', overlay.size, (0, 0, 0, 0))
    ld = ImageDraw.Draw(legend_bg)
    ld.rounded_rectangle([lx - 10, ly - 10, lx + box_w, ly + box_h],
                         radius=8, fill=(0, 0, 0, 120), outline=(100, 100, 100, 100))
    overlay.alpha_composite(legend_bg)

    draw = ImageDraw.Draw(overlay)

    # Title
    draw.text((lx + 5, ly), "RECON MAP", font=font_legend_title, fill=(210, 195, 110, 255))
    y = ly + 36

    # Location icons
    for label, cat, gen_func in legend_items:
        icon = get_icon(cat, 14)
        paste_icon(overlay, icon, lx + 16, y + 8)
        draw = ImageDraw.Draw(overlay)
        lbl_color = LABEL_COLORS.get(cat, WHITE)
        draw.text((lx + 36, y), label, font=font_legend, fill=lbl_color)
        y += row_h

    y += 10
    draw.text((lx + 5, y), "Loot Intel", font=font_legend_title, fill=(210, 195, 110, 255))
    y += 26

    # Collectible icons
    for label, cat in collect_items:
        icon = get_collect_icon(cat, 8)
        paste_icon(overlay, icon, lx + 16, y + 6)
        draw = ImageDraw.Draw(overlay)
        col = COLLECT_COLORS.get(cat, WHITE)
        draw.text((lx + 36, y), label, font=font_legend, fill=col)
        y += row_h
...
def save_dxt1_dds(path, img):
    w, h = img.size
    # Calculate mipmap count (down to 4x4 minimum for DXT1)
    mip_count = 1
    mw, mh = w, h
    while mw > 4 and mh > 4:
        mw //= 2; mh //= 2
        mip_count += 1
    # DDS header flags: CAPS|HEIGHT|WIDTH|PIXELFORMAT|MIPMAPCOUNT|LINEARSIZE
    flags = 0x1 | 0x2 | 0x4 | 0x1000 | 0x20000 | 0x80000
    hdr = bytearray(128)
    hdr[0:4] = b'DDS '
    struct.pack_into('<I', hdr, 4, 124)           # header size
    struct.pack_into('<I', hdr, 8, flags)          # flags
    struct.pack_into('<I', hdr, 12, h)             # height
    struct.pack_into('<I', hdr, 16, w)             # width
    struct.pack_into('<I', hdr, 20, max(1,(w+3)//4)*max(1,(h+3)//4)*8)  # pitch/linearsize
    struct.pack_into('<I', hdr, 28, mip_count)     # mipmap count
    struct.pack_into('<I', hdr, 76, 32)            # pixel format size
    struct.pack_into('<I', hdr, 80, 0x4)           # pixel format flags (FOURCC)
    hdr[84:88] = b'DXT1'                           # fourcc
...
def main():
    print("=" * 60)
    print("  LoreOut Completionist Map Generator v4")
    print("  Clean Custom Icons Edition")
    print("=" * 60)

    # Fonts -- all Bahnschrift, SMALL sizes for clean Aeon-style look
    try:
        font_loc = ImageFont.truetype(r"C:\Windows\Fonts\bahnschrift.ttf", 16)
        font_loc_major = ImageFont.truetype(r"C:\Windows\Fonts\bahnschrift.ttf", 20)
        font_grid = ImageFont.truetype(r"C:\Windows\Fonts\bahnschrift.ttf", 18)
        font_legend = ImageFont.truetype(r"C:\Windows\Fonts\bahnschrift.ttf", 16)
        font_legend_title = ImageFont.truetype(r"C:\Windows\Fonts\bahnschrift.ttf", 18)
        font_watermark = ImageFont.truetype(r"C:\Windows\Fonts\bahnschrift.ttf", 14)
    except Exception as e:
        print(f"  Font warning: {e}")
        font_loc = ImageFont.load_default()
        font_loc_major = font_loc
        font_grid = font_loc
        font_legend = font_loc
        font_legend_title = font_loc
        font_watermark = font_loc

    # Pre-render all icon types
    print("\n  Pre-rendering custom icons...")
    for cat in ICON_GENERATORS:
        sz = ICON_SIZE_MAJOR if cat in ('city', 'vault') else (ICON_SIZE_SMALL if cat in ('poi', 'other') else ICON_SIZE)
        icon = get_icon(cat, sz)
        print(f"    {cat}: {icon.size[0]}x{icon.size[1]}px")
    for cat in COLLECT_GENERATORS:
        icon = get_collect_icon(cat, COLLECT_ICON_SIZE)
        print(f"    {cat} (collect): {icon.size[0]}x{icon.size[1]}px")
    ...
    # STEP 4: Gold dashed grid
    print("\n  Drawing gold dashed grid...")
    ...
    draw_dashed_line(draw, (bx1, by1), (bx2, by1), BORDER_COLOR, width=2, dash_len=30, gap_len=15)
    draw_dashed_line(draw, (bx2, by1), (bx2, by2), BORDER_COLOR, width=2, dash_len=30, gap_len=15)
    draw_dashed_line(draw, (bx2, by2), (bx1, by2), BORDER_COLOR, width=2, dash_len=30, gap_len=15)
    draw_dashed_line(draw, (bx1, by2), (bx1, by1), BORDER_COLOR, width=2, dash_len=30, gap_len=15)
    ...
    # STEP 6: Draw location icons
    print("  Drawing location icons...")
    for m in all_markers:
        cat = m['category']
        sz = ICON_SIZE_MAJOR if cat in ('city', 'vault') else (ICON_SIZE_SMALL if cat in ('poi', 'other') else ICON_SIZE)
        icon = get_icon(cat, sz)
    ...
    # STEP 7: Draw collectible indicators near locations
    print("  Drawing collectible indicators...")
    for loc_idx, categories in collect_groups.items():
        loc = all_markers[loc_idx]
        # Count unique collectible types at this location
        unique_cats = list(set(categories))
        # Position small icons in a row below the location icon
        start_x = loc['px'] - (len(unique_cats) * 7) // 2
        iy = loc['py'] + 16  # below the main icon
        for ci, cat in enumerate(sorted(unique_cats)):
            count = categories.count(cat)
            cicon = get_collect_icon(cat, COLLECT_ICON_SIZE)
            paste_icon(overlay, cicon, start_x + ci * 14, iy)
    draw = ImageDraw.Draw(overlay)

    # STEP 8: Draw location labels
    print("  Drawing location labels...")
    placer = LabelPlacer()
    priority = {'city': 0, 'vault': 1, 'military': 2, 'settlement': 3,
                'metro': 4, 'police': 5, 'other': 99, 'poi': 99}
    labeled = [m for m in all_markers if m['category'] not in ('poi',)]
    labeled.sort(key=lambda m: priority.get(m['category'], 50))

    for m in labeled:
        cat = m['category']
        sz = ICON_SIZE_MAJOR if cat in ('city', 'vault') else (ICON_SIZE_SMALL if cat in ('other',) else ICON_SIZE)

        font = font_loc_major if cat in ('city', 'vault', 'military') else font_loc
        ...
        pos = placer.try_place(label_x, label_y, tw + 4, th + 2)
        if pos:
            draw_text_shadow(draw, pos, text, font, fill=color)

    # STEP 9: Legend
    print("  Drawing legend...")
    draw_legend(overlay, font_legend, font_legend_title)
    draw = ImageDraw.Draw(overlay)

    # STEP 10: Watermark
    draw_text_shadow(draw, (IMG_SIZE - 200, IMG_SIZE - 35), "LOREOUT",
                     font_watermark, fill=(160, 160, 160, 60), shadow=(0, 0, 0, 40))

    # STEP 11: Subtle AA post-process
    print("  Post-processing...")
    overlay = overlay.filter(ImageFilter.GaussianBlur(radius=0.3))

    # STEP 12: Composite and save
    print("\n  Compositing...")
    ...
    print("\n" + "=" * 60)
    print("  MAP v4 GENERATED (Custom Icons)!")
